"替换掉公司名称和作者名称
"company不超过11个字符，否则影响美观，不够的加空格，或者修改用到company的那行代码
let g:verilog_company = "TEST       "
let g:verilog_author_name = "TestName"
...
iabbrev <= <= #1
"iabbrev begin begin //
"iabbrev end end //end for  
"iabbrev if if() begin // 
"iabbrev case case() //synopsys parallel_case
"iabbrev endcase endcase // endcase for

command Aheader :call  AddHeader()
command Allp  :call AddAlways("posedge", "")
...
function AddHeader()
  "call append(0,  "//================================================================================")
  "call append(1,  "// Created by         : g:company")
  "call append(2,  "// Filename           : ".expand("%"))
  "call append(3,  "// Author             : g:author_name")
  ...
  "call append(9, "//================================================================================")
  let curr_line = 0 
  call append(curr_line,    "//+FHDR-------------------------------------------------------------------------")
  call append(curr_line+1,  "//--            ******************************************                      ")
  call append(curr_line+2,  "//--            ** Copyright (C) 2009,".g:verilog_company.", Inc. **                      ")
  call append(curr_line+3,  "//--            ** All Rights Reserved.                 **                      ")
  call append(curr_line+4,  "//--            ******************************************                      ")
  call append(curr_line+5,  "//------------------------------------------------------------------------------")
  call append(curr_line+6,  "//-- Module Name     :    ".expand("%:r"))
  call append(curr_line+7,  "//-- Hierarchy       :    level1_submodule - * - ".expand("%:r"))
  call append(curr_line+8,  "//-- Description     :    Interface                                             ")
  call append(curr_line+9,  "//                                                                              ")
  call append(curr_line+10, "//-- Last Modified   :                                                           ")
  call append(curr_line+11, "//-- Revision history:                                                          ")
  call append(curr_line+12, "//     Date               Author       Description                              ")
  call append(curr_line+13, "//     ".strftime("%Y-%m-%d %H:%M")."   ".g:verilog_author_name."          Initialize code")
  call append(curr_line+14, "//-FHDR-------------------------------------------------------------------------")
endfunction
"===============================================================
"
"===============================================================
...
function AddAlways(clk_edge, rst_edge)
   for line in getline(1, line("$"))
      ...
      elseif line =~ '^\s*\<reg\>.*//\s*\<reset\>\s*$'
         let line = substitute(line, '^\s*\<reg\>\s*', "", "")
         let rst = substitute(line, '\s*;.*$', "", "")
      elseif line =~ '^\s*\<input\>.*,\s*//\s*\<clock\>\s*$'
         let line = substitute(line, '^\s*\<input\>\s*', "", "")
         let clk = substitute(line, '\s*,.*$', "", "")
      elseif line =~ '^\s*\<input\>.*,\s*//\s*\<reset\>\s*$'
         let line = substitute(line, '^\s*\<input\>\s*', "", "")
...
fun LastModified()
  "let l = 0
  " for line in getline(1, line("$"))
  "    if line =~ '^//\s*Date\s*Author\s*Description\s*$'
  "      call append(l+1, "//     ".strftime("%Y-%m-%d %H:%M")."   ")
  "    endif
  "    let l = l + 1
  " endfor
"        call append(13, "//     ".strftime("%Y-%m-%d %H:%M")."   PythonWang   Initialize code                          ")
